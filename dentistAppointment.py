# DentistAppointmentScheduler System - Main Application
...
def register():
    ...
    if request.method == "POST" and form.validate():
        ...
        if same_ssn:
            flash(message="This SSN has been registered, if you think there was a mistake then contact 'Team B'",
                  category="danger")
            cursor.close()
            return redirect(url_for("index"))

        if same_email:
            flash(message="This email has been registered, if you think there was a mistake then contact 'Team B'",
                  category="danger")
            cursor.close()
            return redirect(url_for("index"))
...
def profile():
    ...
    if is_address_exist and is_phone_exist and not session["is_patient"]:
        ...
        cursor.close()
        flash("You have successfully activated your account. Now you can use Dental appointment system.", category="success")
...
def book_an_appointment():
    ...
    if request.method == "GET":
        ...
        form.dentist.choices = [(dentist["dentist_id"], dentist["first_name"] + " " + dentist["last_name"]) for dentist
                                in
                                dentists]
        # form.hour.choices = [('8.15', '8.15 am'), ('9.15 pm', '9.15 pm'), (10, 10), (11, 11), (12, 12), (15, 15), (16, 16)]
        form.hour.choices = [(8, '8:00 am'), (9, '9:00 am'), (10, '10:00 am'), (11, '11:00 am'),
                              (12, '12:00 am'), (14, '2:00 pm'), (15, '3:00 pm'), (16, '4:00 pm')]
...
def add_holiday():
    ...
    if request.method == "POST" and form.validate():
        ...
        query = """SELECT *
               ...
               ON holiday_dates.resting_id = dentists.dentist_id and rest_date = %s
               WHERE dentist_ssn = %s"""
        formatted_date = rest_date.strftime('%Y-%m-%d')
        holiday_date = cursor.execute(query, (formatted_date, session["ssn"]))
        if holiday_date:
            cursor.close()
...
def dentist_statistics():
    ...
    query = """ select * from dentists as d inner join (SELECT  d_id, COUNT(d_id) AS magnitude 
                    FROM appointments
                    LEFT OUTER JOIN dentists
                    ...
                    GROUP BY d_id
                    ORDER BY magnitude DESC
                    LIMIT 10) as a  on d.dentist_id= a.d_id 
                    inner join persons as p on p.ssn = d.dentist_ssn"""
    cursor.execute(query)
    ...
    query = """ select * from dentists as d inner join (SELECT appointments.d_id, COUNT(appointments.d_id) AS magnitude 
                        FROM treatments
                        LEFT OUTER JOIN appointments
                        ON treatments.a_id = appointments.appointment_id
                        LEFT OUTER JOIN dentists
...
def patient_statistics():
    ...
    cursor = mysql.connection.cursor()
    query = """ SELECT treatments.complaint, COUNT(treatments.complaint) AS magnitude 
                FROM treatments
                LEFT OUTER JOIN appointments
                ON treatments.a_id = appointments.appointment_id
                LEFT OUTER JOIN patients
    ...
    query = """ SELECT treatments.a_id, MAX(treatments.charge) as charge FROM treatments  GROUP BY treatments.charge  ,   treatments.a_id  ORDER BY treatments.charge DESC  """
    cursor.execute(query)
